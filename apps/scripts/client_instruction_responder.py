...
- 画像ファイルを読み込む <img> タグは使わず、JavaScript/CSS のレイアウトと装飾だけで可視化してください。
...
def build_blog_generation_prompt(
    ...
    return (
        ...
        "  - 中盤: 仕組み・フロー・アーキテクチャ・比較・グラフ（フロー図、シーケンス図、全体アーキテクチャ図、インフラ構成図、比較表、レーダーチャート、折れ線/棒/円グラフ、ステップ図）。\n"
        "  - 終盤: リスク図解・要点サマリー・今後の展望（リスクマトリクス、Myth vs Fact 図、要点サマリー図、今後の展望ツリー、ペルソナ/ユースケースのカード）。\n"
        "- 図解は純粋な HTML/CSS で簡潔に表現し、内容を短い箇条書きやテーブルで示してください (SVG/Canvas 不要・<img> での画像貼り付け禁止)。\n"
        "\n[出力HTML構造]\n"
        "- <article> 内に収め、プレーン HTML で返してください。\n"
        "- <header> ではタイトルを <h1> で表示し、その下に約 200 文字のディスクリプションとタグ 6 つ (ul > li) を配置してください。\n"
        "- 概論・各セクション・総論の見出しはすべて <h2> を使い、図解や補足のサブセクションがある場合は <h3> を使って階層を明示してください。\n"
        "- <section class=\"summary\"> に 300〜400 文字のリード文と、重要ポイント 3〜5 件の箇条書きを置いてください。\n"
        "- セクション名を 7 つ使い、各 <section class=\"body\"> に <h2> 見出し＋本文 350〜500 文字＋ 3 点以内の箇条書きを含め、本文直後に対応する <figure> 図解を挿入してください。\n"
        "- <footer> で総論ブロックを <h2> 付きで設け、リスクと対処法の要約、今後の展望や ToDo を簡潔にまとめてください。\n"
        "\n[執筆要件]\n"
