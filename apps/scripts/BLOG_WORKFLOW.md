...
## 生成手順
1. **概論 (500 文字程度)**
   1. `index.json` 全体と `knowledge.md` の要点を読み、ブログ全体の背景と狙いを 500 文字前後の日本語で要約するように LLM に依頼します。
   2. 「索引用のエントリを横断し、読者像にフィットする導入にする」と明示すると、セクション本文とのつながりが良くなります。

2. **セクション本文 (各 1,500 文字程度)**
   1. セクション見出しやリード文をキーに `index.json` を照合し、最も近いエントリの `summary` を詳細素材として抽出します。
   2. `knowledge.md` から Johanne の嗜好・不安・期待に触れる記述を併用し、読者文脈を補強します。
...
## スクリプトからの使い方
このファイル自体はコードから自動で読み込まれるものではなく、プロンプトを組むときの「手元メモ」として参照します。以下のように運用してください。

1. 概論を作るときは、本書の「参照するデータ」と「生成手順」のセクションを確認し、`client_instruction_responder.py` に渡すプロンプトにインデックスと知識ベースをどう含めるかを決めます。
2. セクション本文を生成する際は、`blog_server.py` から `client_instruction_responder.py` に流れる `prompt` を本書のテンプレートを元に組み立て、セクションのキー文（`section_lead` など）と照合した `indexes/index.json` のサマリー、`mybrain/knowledge.md` の抜粋を付けて送ります。
3. スクリプト側で追加するのは文字列の組み立てのみで、ファイルの読み込みやパースは必要に応じてスクリプト内で実装してください（本書は参照先と流れの指針を示すのみです）。
