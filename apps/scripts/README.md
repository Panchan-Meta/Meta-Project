...
ブログの概論（約 500 文字）とセクション本文（各 1,500 文字）を生成する際のプロンプト設計とデータ参照手順をまとめたドキュメントを追加しました。`indexes/` と `indexes/mybrain/` にあるテキストファイルを組み合わせて LLM に渡すワークフローを確認したいときは、[`BLOG_WORKFLOW.md`](./BLOG_WORKFLOW.md) を参照してください。
